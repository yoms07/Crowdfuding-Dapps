...
export class KBToken extends ethereum.SmartContract {
  static bind(address: Address): KBToken {
    return new KBToken("KBToken", address);
  }
  ...
  topUp(to: Address, amount: BigInt): boolean {
    let result = super.call("topUp", "topUp(address,uint256):(bool)", [
  ...
  try_topUp(to: Address, amount: BigInt): ethereum.CallResult<boolean> {
    let result = super.tryCall("topUp", "topUp(address,uint256):(bool)", [
      ethereum.Value.fromAddress(to),
      ethereum.Value.fromUnsignedBigInt(amount),
    ]);
    if (result.reverted) {
